...
// Inline SVG Graduation Cap Icon
...
export default function Education() {
  ...
  return (
    <section id="education" className="w-full my-16 px-4 py-16 bg-white text-gray-900 rounded-xl shadow-lg">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative">
          ...
          {educationData.map((edu, index) => (
            <div key={index} className={`mb-10 flex flex-col md:flex-row items-start w-full ${index % 2 === 0 ? 'md:justify-start' : 'md:justify-end'}`}>
              
              {/* Card */}
              <div className={`bg-white p-6 rounded-xl shadow-lg hover:shadow-2xl transition duration-500 w-full md:w-[45%] border border-gray-100
                ...
                <h4 className="text-gray-900 font-extrabold text-xl sm:text-2xl mb-1">{edu.degree}</h4>
                <p className="text-purple-700 font-medium text-base sm:text-lg mb-3">{edu.institution}</p>
                <p className="text-gray-700 text-sm leading-relaxed">{edu.description}</p>
              </div>

              {/* Timeline Dot */}
              <div className="absolute left-1/2 top-0 transform -translate-x-1/2 w-4 h-4 rounded-full bg-purple-600 ring-4 ring-white shadow-md hidden md:block"></div>
            </div>
          ))}
        </div>
      </div>
