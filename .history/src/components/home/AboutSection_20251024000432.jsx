    ...
    <section className="bg-white text-black py-16 px-4 sm:px-6 lg:px-12">
      <div className="container mx-auto flex flex-col md:flex-row items-center gap-12">
        ...
        >
          ...
          {/* Features List */}
          {/* 🔹 CHANGE 1: Added horizontal scroll for mobile using overflow-x-auto */}
          {/* <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 gap-4 mb-8 md:overflow-visible overflow-x-auto scroll-px-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
            {features.map((feature, i) => (
              <li
                key={i}
                className="flex min-w-[280px] sm:min-w-auto items-start gap-3 bg-gray-50 hover:bg-blue-50 p-3 sm:p-4 rounded-xl shadow-sm hover:shadow-md transition duration-300"
              >
                {feature.icon}
                <span className="text-gray-800 text-sm sm:text-base">{feature.text}</span>
              </li>
            ))}
          </ul> */}
